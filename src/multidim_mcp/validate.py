...
from .frames import normalize_statement as frames_normalize
...
# Upper bound on the alternatives list: the duplicate check below compares
# every pair, so the cost grows quadratically. Well past any real analysis.
MAX_ALTERNATIVES = 50

# Same reasoning for the pre-mortem-vs-risks pass, which compares every
# pre-mortem text to every risk field: the pair count is the product of the two
# lists. Unbounded, a 60 KB frame of small items burned tens of seconds of
# server time -- a stall any client could trigger. Both sides are capped and an
# oversize list is refused outright instead of being compared.
MAX_RISKS = 50
MAX_PREMORTEM_TEXTS = 50

VERDICT_ORDER = {"ACCEPT": 0, "WARNING": 1, "REJECT": 2}


def _jaccard(ta, tb) -> float:
    """Jaccard on ALREADY tokenised sets, so a pairwise pass can fold each side
    once instead of re-folding both texts inside every comparison."""
    if not ta or not tb:
        return 0.0
...
def similarity(a, b) -> float:
    ...
    if not isinstance(b, str):
        b = ""
    return _jaccard(tokens_of(fold(a)), tokens_of(fold(b)))


def ordered_tokens(s):
    """ORDERED sequence of folded tokens (keeps order and repetitions, unlike
    ``tokens_of`` which returns a set). Comparison operators survive as their
    own tokens -- same reason as ``frames.normalize_statement``: 'x < 3' and
    'x > 3' are OPPOSITE ideas and must never be judged normalized-equal
    (that verdict is a REJECT)."""
    # normalise ONCE: calling frames_normalize twice doubled the cost of every
    # comparison in the pairwise passes below
    normalised = frames_normalize(s)
...
def _compare_premortem_to_risks(ptexts, risks, report) -> None:
    """Report a pre-mortem that copies (REJECT) or echoes (WARNING) a risk.

    Both lists are bounded by the caller, because this compares every text on
    one side to every field on the other. Each side is tokenised ONCE here:
    re-folding both texts inside every pair made the work quadratic on top of
    the pair count, and a 60 KB frame of small items stalled the server for
    tens of seconds.
    """
    p_tokens = [tokens_of(fold(pt)) for pt in ptexts]
    p_norms = [frames_normalize(pt) for pt in ptexts]
    for i, r in enumerate(risks):
        ...
        for fld in ("risk", "first_order_effect", "second_order_effect", "mitigation"):
            ...
            if isinstance(v, str) and v.strip():
                candidates.append(v)
        c_tokens = [tokens_of(fold(cand)) for cand in candidates]
        c_norms = [frames_normalize(cand) for cand in candidates]
        sim = max(_jaccard(pt, ct) for pt in p_tokens for ct in c_tokens)
        if any(pn and pn == cn for pn in p_norms for cn in c_norms):
...
def validate_analysis(frame, analysis):
    ...
    if isinstance(min_a, int) and alternatives:
        # count DISTINCT ideas, not list entries: copying one alternative
        # under a fresh identifier must not satisfy the minimum
        distinct_keys = set()
        for alt in alternatives:
            stmt = alt.get("statement", "") if isinstance(alt, dict) else str(alt)
            if isinstance(stmt, str):
                key = frames_normalize(stmt)
                if key:
                    distinct_keys.add(key)
        if len(distinct_keys) < min_a:
            report("alternatives", "REJECT", "NOT_ENOUGH_ALTERNATIVES",
                   "{} distinct alternative(s), minimum {} at this depth".format(
                       len(distinct_keys), min_a))
    ...
    # 4b. alternatives must also differ from EACH OTHER: copying A1 into A2
    # under a fresh identifier earned an ACCEPT (only the vs-primary pair was
    # checked). Same bar as vs-primary: manifest duplication = REJECT,
    # ambiguous similarity = WARNING.
    # A section is an ANALYSIS, not a data dump: the pairwise pass is O(n^2),
    # so an oversized list is refused outright rather than burning seconds on
    # a comparison nobody asked for.
    if len(alternatives) > MAX_ALTERNATIVES:
        report("alternatives", "REJECT", "TOO_MANY_ALTERNATIVES",
               "{} alternatives, maximum {}: an analysis lists distinct options, "
               "not a dump".format(len(alternatives), MAX_ALTERNATIVES))
    elif "ALTERNATIVE_DUPLICATES_PRIMARY" in rules and len(alternatives) > 1:
        def _alt_label(alt, idx):
            return ((alt.get("alternative_id") if isinstance(alt, dict) else None)
                    or "#{}".format(idx + 1))

        for i in range(len(alternatives)):
            stmt_i = (alternatives[i].get("statement", "")
                      if isinstance(alternatives[i], dict) else str(alternatives[i]))
            if not isinstance(stmt_i, str):
                continue  # STATEMENT_NOT_TEXT already reported above
            for j in range(i + 1, len(alternatives)):
                stmt_j = (alternatives[j].get("statement", "")
                          if isinstance(alternatives[j], dict) else str(alternatives[j]))
                if not isinstance(stmt_j, str):
                    continue
                sim = similarity(stmt_i, stmt_j)
                if normalized_equal(stmt_i, stmt_j):
                    report("alternatives", "REJECT", "ALTERNATIVE_DUPLICATES_ALTERNATIVE",
                           "alternatives {} and {} are the same idea".format(
                               _alt_label(alternatives[i], i), _alt_label(alternatives[j], j)))
                elif sim >= SIMILARITY_WARN:
                    report("alternatives", "WARNING", "ALTERNATIVES_SIMILAR",
                           "alternatives {} and {} are close (similarity {:.2f})".format(
                               _alt_label(alternatives[i], i), _alt_label(alternatives[j], j), sim))
                ...
                if isinstance(v, str) and v.strip():
                    ...
                    for x in v:
                        if isinstance(x, str) and x.strip():
                            ptexts.append(x)
        # this pass compares EVERY pre-mortem text to EVERY risk field, so the
        # work is the product of both lists. Refuse an oversize side outright
        # rather than spend seconds of server time on a comparison nobody
        # asked for -- same rule as the alternatives above.
        if len(risks) > MAX_RISKS:
            report("second_order_risks", "REJECT", "TOO_MANY_RISKS",
                   "{} risks, maximum {}: an analysis argues, it does not dump"
                   .format(len(risks), MAX_RISKS))
        elif len(ptexts) > MAX_PREMORTEM_TEXTS:
            report("premortem", "REJECT", "PREMORTEM_TOO_LARGE",
                   "{} pre-mortem entries, maximum {}: an analysis argues, it "
                   "does not dump".format(len(ptexts), MAX_PREMORTEM_TEXTS))
        else:
            _compare_premortem_to_risks(ptexts, risks, report)
