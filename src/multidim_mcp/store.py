...
portable path** (see :mod:`multidim_mcp.paths`) -- never ``~/.multidim``.
...
def _reject_json_constant(name):
    ...
    Python accepts them by default, so a hand-edited store could hold a
    value that no strict parser can read back -- and that a frame would
    then carry onto the wire. Treat it as CORRUPTION: the load path
    catches this ValueError, backs the file up and resets."""
    raise ValueError("invalid JSON constant in store: %s" % name)


def _reject_non_finite_float(text):
    """``parse_float`` hook: a numeric LITERAL that overflows to infinity.

    ``1e999`` is valid JSON syntax, so ``parse_constant`` never sees it, yet
    Python decodes it to ``inf`` -- which a frame would then carry onto the
    wire where no strict parser can read it. Same treatment as NaN: the load
    path catches this ValueError, backs the file up and resets.
    """
    value = float(text)
    if value != value or value in (float("inf"), float("-inf")):
        raise ValueError("non-finite number in store: %s" % text)
    return value
...
def _is_generic_name(name) -> bool:
    """The seed 'generic' identity, normalised (strip + casefold).

    find_context resolves names that way, so the presence check must agree:
    a store holding ' Generic ' was FOUND by lookups yet failed the exact
    'generic' check, triggering a reset that destroyed the user's own
    contexts.
    """
    return isinstance(name, str) and name.strip().casefold() == "generic"


def _has_unique_names(contexts) -> bool:
    """True when no two contexts share a NORMALISED name.

    find_context resolves by stripped+casefolded name and returns the FIRST
    match, while detect_context scores every entry and can pick the SECOND.
    The two would then disagree about which context a frame belongs to, and
    validate would reject an untouched frame. Duplicate identities are
    unrecoverable corruption: back up and reset.
    """
    keys = [str(c.get("name", "")).strip().casefold()
...
def _valid_context(c) -> bool:
    ...
    if "traps" in c:
        ...
        if not isinstance(traps, list) or not all(_valid_trap(t) for t in traps):
            return False
        # trap_id is a LOOKUP key, so it must be unique inside a context. With
        # a duplicate on disk, disabling that id only reaches whichever twin
        # the index happens to hold and select_traps keeps injecting the
        # other: a lesson the user switched off comes back. Hand-edited
        # corruption -- reject it here, like a duplicate context name.
        ids = [t["trap_id"] for t in traps]
        if len(set(ids)) != len(ids):
            return False
        # ...and the LESSON must be unique too, not just its id. upsert_traps
        # dedups by normalised statement, so two distinct ids carrying one
        # idea can only come from a hand edit -- and select_traps would then
        # inject the SAME mandatory question twice into every frame.
        lessons = [frames.normalize_statement(t["statement"]) for t in traps]
...
def _open_new_backup(base: Path, limit: int = 100) -> Tuple[int, Path]:
    """Open a brand-new backup file next to the store, never an existing one.

    A store can go corrupt TWICE. With one fixed name, the second O_EXCL open
    failed and load() raised for good: the branch meant to recover from
    corruption became the thing that made recovery impossible, and the server
    was unusable until someone deleted the file by hand. Suffix the name until
    a free one is found, keeping O_EXCL -- so a symlinked or pre-existing path
    is still never written through -- and refuse once the limit is reached
    rather than overwrite the oldest evidence.
    """
    for index in range(limit):
        candidate = (base if index == 0
                     else base.with_name("{}.{}".format(base.name, index)))
        paths.assert_not_personal(candidate)
        try:
            fd = os.open(str(candidate),
                         os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
        except (FileExistsError, IsADirectoryError, PermissionError):
            # 'this name is taken' reaches us differently per platform: EEXIST
            # on POSIX, EACCES on Windows when a DIRECTORY sits there. Treat
            # all three as taken and try the next name; any other OSError (a
            # full disk, say) still propagates at once.
            continue
        return fd, candidate
    raise OSError("{} backups already sit next to {}: clean them up".format(
        limit, base.name))


def _atomic_write(path: Path, data: Dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    # allow_nan=False: NaN/Infinity are a Python extension, not JSON --
    # writing one would produce a store no strict parser can read
    # ensure_ascii=True: a lone surrogate anywhere in the data (a learned
    # description, say) is valid JSON input but NOT encodable in UTF-8 --
    # the write raised UnicodeEncodeError and the tool answered an internal
    # error. Escaped, it round-trips like any other text.
    text = json.dumps(data, ensure_ascii=True, indent=2, allow_nan=False)
...
def _read_valid_no_migrate(path: Path) -> Optional[Dict]:
    ...
    try:
        ...
        return None
    try:
        # invalid UTF-8 is CORRUPTION, not an I/O hiccup: fall through to the
        # locked path so the file is backed up (original bytes) then reset,
        # instead of crashing load() with an uncaught UnicodeDecodeError
        raw = raw_bytes.decode("utf-8")
        parsed = json.loads(raw, parse_constant=_reject_json_constant,
                            parse_float=_reject_non_finite_float)
    except (ValueError, TypeError, RecursionError):
        # RecursionError: a deeply nested store blows the decoder's recursion
        # limit. Still corruption, not an I/O fault -> defer to the locked
        # path, which backs the file up before resetting.
        return None
    ...
    if not isinstance(contexts, list) or not all(_valid_context(c) for c in contexts):
        return None
    if not _has_unique_names(contexts):
        return None
    if not any(_is_generic_name(c.get("name")) for c in contexts):
        return None
...
def _ensure_store_locked(path: Path):
    ...
    try:
        # decoding is part of the CORRUPTION check: invalid UTF-8 bytes must
        # take the backup+reset path below (UnicodeDecodeError is a ValueError),
        # never crash load() without a backup
        raw = raw_bytes.decode("utf-8")
        parsed = json.loads(raw, parse_constant=_reject_json_constant,
                            parse_float=_reject_non_finite_float)
        if not isinstance(parsed, dict):
        ...
        if not _has_unique_names(contexts):
            raise ValueError("two contexts share the same normalised name")
        if not any(_is_generic_name(c.get("name")) for c in contexts):
        ...
        # RecursionError: nesting too deep for the decoder -> same reset
        bak = path.with_name(path.name + ".bak")
        try:
            # SECURITY: never write THROUGH an existing backup path. A
            # symlinked store.json.bak pointing at a personal store would
            # otherwise be overwritten with our evidence dump. O_EXCL creates
            # a brand-new file or fails; the personal-store guard covers the
            # resolved path too. Write the ORIGINAL bytes: a text re-encode
            # would be impossible for invalid UTF-8 (the very case landing
            # here) and could alter the evidence being preserved.
            # 0o600 explicitly: this backup holds the SAME user content as the
            # store, and tempfile.mkstemp already gives store.json 0o600 (the
            # mode os.replace carries over). Without a mode, os.open falls back
            # to 0o777 & ~umask -- typically 0o755, world-readable -- so the
            # evidence dump would be more exposed than what it copies.
            fd, bak = _open_new_backup(bak)
            try:
                # os.write is allowed to write FEWER bytes than asked. Taking
                # it for granted accepted a truncated backup and then wiped the
                # original -- the one thing this whole branch exists to
                # prevent. Loop to completion, treat a 0-byte write as failure
                # (no progress), and fsync before the reset below destroys the
                # only other copy.
                written = 0
                while written < len(raw_bytes):
                    n = os.write(fd, raw_bytes[written:])
                    if n <= 0:
                        raise OSError("short write: %d of %d bytes backed up"
                                      % (written, len(raw_bytes)))
                    written += n
                os.fsync(fd)
            finally:
                os.close(fd)
        except (OSError, RuntimeError) as werr:
...
def _file_lock(lock_path: Path, timeout: float = LOCK_TIMEOUT_S):
    ...
    # the lock file is a WRITE too: a symlinked store.json.lock pointing into
    # a personal store would have it created there, outside the tripwire.
    # Guard the resolved path, and refuse to follow a symlink where the OS
    # lets us (O_NOFOLLOW is POSIX-only; Windows has no equivalent flag, so
    # the resolved-path guard carries that platform).
    paths.assert_not_personal(lock_path)
    flags = os.O_CREAT | os.O_RDWR | getattr(os, "O_NOFOLLOW", 0)
    fd = os.open(str(lock_path), flags, 0o600)  # same restriction as the store
    try:
...
def find_context(store: Dict, name: str) -> Optional[Dict]:
    """Look a context up by name, stripped and case-folded.

    Both sides are normalised: a stored 'legacy_ctx ' (trailing space, from a
    hand-edited store) passes validation, so a lookup for 'legacy_ctx' must
    find it -- otherwise learn believes the context is new and appends a
    duplicate. ``casefold`` (not ``lower``) so non-ASCII names compare the way
    their speakers expect.
    """
    key = name.strip().casefold()
    for c in store.get("contexts", []):
        if str(c.get("name", "")).strip().casefold() == key:
            return c
    return None
