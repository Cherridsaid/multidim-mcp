...
def _extra_forbidden_tokens() -> list:
    ...
    ``MULTIDIM_MCP_EXTRA_FORBIDDEN`` env var (comma-separated). This lets a private
    ...
    raw = os.environ.get("MULTIDIM_MCP_EXTRA_FORBIDDEN", "")
...
def assert_neutral(contexts=None) -> None:
    ...
    ``MULTIDIM_MCP_EXTRA_FORBIDDEN`` (see :func:`_extra_forbidden_tokens`).
    """
    if contexts is None:
