...
from multidim_mcp import base_contexts, paths, store
    ...
    def test_override_into_personal_multidim_is_refused(self):
        ...
        for target in (personal, os.path.join(personal, "sub", "deeper")):
            with self.subTest(target=target):
                os.environ["MULTIDIM_MCP_HOME"] = target
                with self.assertRaises(RuntimeError):
                    paths.store_path()
...
class TestFirstRunAndPersistence(StoreTestBase):
    ...
    def test_invalid_utf8_store_backed_up_bytes_then_reset(self):
        # Review finding (2026-07-27, extraction): raw invalid bytes (b'\xff')
        # raised UnicodeDecodeError BEFORE the corruption path -- load()
        # crashed and no .bak was written. Invalid encoding is corruption:
        # the ORIGINAL bytes are backed up, then the store resets to seeds.
        p = paths.store_path()
        p.parent.mkdir(parents=True, exist_ok=True)
        garbage = b"\xff\xfe not utf8 \x80\x81"
        p.write_bytes(garbage)
        s = store.load()
        bak = p.with_name(p.name + ".bak")
        self.assertTrue(bak.exists())
        self.assertEqual(bak.read_bytes(), garbage)  # evidence preserved verbatim
        self.assertIn("generic", store.list_context_names(s))
        self.assertIn("_reset_reason", s)
    ...
    def test_occupied_backup_name_does_not_block_recovery(self):
        ...
        occupied = p.with_name(p.name + ".bak")
        occupied.mkdir()
        reloaded = store.load()
        self.assertIn("_reset_reason", reloaded)
        self.assertTrue(occupied.is_dir())  # never written through
        self.assertNotEqual(Path(reloaded["_backup"]), occupied)
        self.assertEqual(Path(reloaded["_backup"]).read_text(encoding="utf-8"),
                         original)

    def test_corrupt_store_without_any_backup_raises_and_keeps_data(self):
        # The original guarantee, unchanged: if NO backup can be written at
        # all, the corrupt store must not be overwritten -- fail closed, never
        ...
        with mock.patch.object(store, "_open_new_backup",
                               side_effect=OSError("no backup possible")):
            with self.assertRaises(RuntimeError):
                store.load()
...
class TestConcurrency(StoreTestBase):
    ...
    def test_lock_is_exclusive_and_times_out(self):
        from multidim_mcp.store import _file_lock
...
class TestNeutrality(unittest.TestCase):
    ...
    def test_private_denylist_extends_the_guard(self):
        ...
        # MULTIDIM_MCP_EXTRA_FORBIDDEN denylist must still catch it
        poisoned = base_contexts.base_contexts()
        ...
        prev = os.environ.get("MULTIDIM_MCP_EXTRA_FORBIDDEN")
        os.environ["MULTIDIM_MCP_EXTRA_FORBIDDEN"] = "acme-secret-codename, other"
        try:
            with self.assertRaises(AssertionError):
                base_contexts.assert_neutral(poisoned)
        finally:
            if prev is None:
                os.environ.pop("MULTIDIM_MCP_EXTRA_FORBIDDEN", None)
            else:
                os.environ["MULTIDIM_MCP_EXTRA_FORBIDDEN"] = prev
