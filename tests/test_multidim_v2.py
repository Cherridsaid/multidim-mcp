...
from multidim_mcp import base_contexts, frames, server, store
from multidim_mcp import paths
...
class TestValidate(_IsolatedStoreCase):
    ...
    def test_oversized_risks_are_refused_before_the_pairwise_pass(self):
        ...
        # client could trigger. Both sides are capped now.
        from multidim_mcp.validate import MAX_RISKS
        frame = self.analyze_v2("a plain subject", context="generic", depth="deep")
        analysis = _golden_deep_analysis()
        template = analysis["second_order_risks"][0]
        analysis["second_order_risks"] = [
            dict(template, risk_id="R%d" % i, risk="distinct risk number %d" % i)
            for i in range(MAX_RISKS + 1)]
        verdict = json.loads(self.validate(frame, analysis)[0])
        self.assertEqual(verdict["verdict"], "REJECT")
        codes = {c for r in verdict["section_results"] for c in r["error_codes"]}
        self.assertIn("TOO_MANY_RISKS", codes)

    def test_oversized_premortem_is_refused_too(self):
        from multidim_mcp.validate import MAX_PREMORTEM_TEXTS
        frame = self.analyze_v2("a plain subject", context="generic", depth="deep")
        analysis = _golden_deep_analysis()
        analysis["premortem"] = {
            "story": "six months later the rollout stalled on an unrelated cause",
            "signals": ["observable signal number %d in the application logs" % i
                        for i in range(MAX_PREMORTEM_TEXTS + 1)],
        }
        verdict = json.loads(self.validate(frame, analysis)[0])
        self.assertEqual(verdict["verdict"], "REJECT")
        codes = {c for r in verdict["section_results"] for c in r["error_codes"]}
        self.assertIn("PREMORTEM_TOO_LARGE", codes)

    def test_worst_allowed_size_validates_quickly(self):
        # the bound is only useful if what it ALLOWS stays cheap
        from multidim_mcp.validate import MAX_RISKS, MAX_PREMORTEM_TEXTS
        frame = self.analyze_v2("a plain subject", context="generic", depth="deep")
        analysis = _golden_deep_analysis()
        template = analysis["second_order_risks"][0]
        analysis["second_order_risks"] = [
        ...
        analysis["premortem"] = {
            "story": "six months later the rollout stalled on an unrelated cause",
            "signals": ["observable signal number %d in the application logs" % i
                        for i in range(MAX_PREMORTEM_TEXTS - 2)],
        }
        started = time.monotonic()
        self.validate(frame, analysis)
        # generous: the point is the ORDER of magnitude, not a benchmark
    ...
    def test_stripped_contract_is_refused_as_tampered(self):
        ...
        # against the rebuilt frame answered as if the tampering never
        # happened, hiding it from the caller.
        frame = self.analyze_v2("a plain subject", context="generic", depth="core")
...
class TestLearnV2(_IsolatedStoreCase):
    ...
    def test_failed_save_leaves_store_unchanged(self):
        ...
        before = json.dumps(self.store, sort_keys=True)
        with mock.patch("multidim_mcp.store.save",
