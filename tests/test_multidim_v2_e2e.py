...
* **E2E over stdio**: drive the REAL entry point (``python -m multidim_mcp``)
...
from multidim_mcp import base_contexts, frames, store
...
def _handshake():
    return [
        {"jsonrpc": "2.0", "id": 0, "method": "initialize", "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "multidim-mcp-e2e-test", "version": "1.0.0"},
        }},
...
def _run_server(requests, home, fake_home=None):
    ...
    isolates the store under a temp MULTIDIM_MCP_HOME. ``fake_home`` (when given)
    ...
    env = dict(os.environ)
    if home is not None:
        env["MULTIDIM_MCP_HOME"] = str(home)
    else:
        ...
        for var in ("MULTIDIM_MCP_HOME", "LOCALAPPDATA", "APPDATA", "XDG_DATA_HOME"):
            env.pop(var, None)
...
class TestStdioE2E(unittest.TestCase):
    ...
    def test_learn_persists_isolated_and_never_touches_personal_store(self):
        ...
        with tempfile.TemporaryDirectory(prefix="phases-fakehome-") as fake_home:
            ...
            # the store was written under the isolated MULTIDIM_MCP_HOME ...
            store_files = list(Path(self.home).rglob("store.json"))
            self.assertTrue(store_files, "learn did not persist under MULTIDIM_MCP_HOME")
    ...
    def test_default_path_without_override_never_touches_personal_store(self):
        # NO MULTIDIM_MCP_HOME: the DEFAULT data dir is used. Even then, with the
        ...
        with tempfile.TemporaryDirectory(prefix="phases-fakehome-") as fake_home:
            ...
            # ... in the dedicated multidim-mcp dir, never in a .multidim dir
...
class TestConsoleScript(unittest.TestCase):
    def test_console_entry_point_is_declared_and_callable(self):
        # __main__ advertises a multidim-mcp console script; it must exist in
        ...
        self.assertEqual(scripts.get("multidim-mcp"),
                         "multidim_mcp.__main__:main")
        from multidim_mcp.__main__ import main as md_main
