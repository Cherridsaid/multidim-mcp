...
from multidim_mcp import base_contexts, server
...
class TestProtocol(unittest.TestCase):
    ...
    def test_tool_call_reloads_store_from_disk(self):
        ...
        from multidim_mcp import store as store_mod
...
class TestAnalyze(unittest.TestCase):
    def setUp(self):
        # handle_message RELOADS the store from disk before every tools/call,
        # so passing a neutral store in memory is not enough: without an
        # isolated MULTIDIM_MCP_HOME these tests read the real data directory
        # and a locally learned context could hijack their assertions.
        self._tmp = tempfile.TemporaryDirectory(prefix="mdmcp-analyze-")
        self._prev = os.environ.get("MULTIDIM_MCP_HOME")
        os.environ["MULTIDIM_MCP_HOME"] = self._tmp.name
        self.store = _neutral_store()
